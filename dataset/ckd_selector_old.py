        ...
        if self.train:
            if self.data_ckd is not None:
                ...
                sample_ckd, _ = transform_apply(Image.fromarray(sample), self.test_transform, \
                                                                 aug_parameters=self.aug_parameters[index], compress=True)
                sample, _ = transform_apply(Image.fromarray(sample), self.test_transform, \
                                                                 aug_parameters=self.aug_parameters[index], compress=False)
...
class CKD_selector_parallel(object):
    ...
    def CKD_operator(self):
        ...
        with torch.no_grad():
            for idx, (input ,target) in enumerate(ckd_loader):
                ...
                ed = st +  len(input)
                self.DataBatch_CKD[st:ed], self.DataBatch_ORG[st:ed], self.aug_parameters_CKD[st:ed] = self.groupCal(input, target, len(target))      
    ...
    def createContainers(self):        
        ...
        self.data_tensor = torch.zeros(self.ckd_batch_size * (len(self.QF_range)+1), self.num_channels, self.height, self.width, dtype=torch.float32)
        ...
        self.DataBatch_CKD =  torch.zeros(self.dataset_size, self.num_channels, self.height, self.width, dtype=torch.float32)
        self.DataBatch_ORG =  torch.zeros(self.dataset_size, self.num_channels, self.height, self.width, dtype=torch.float32)
